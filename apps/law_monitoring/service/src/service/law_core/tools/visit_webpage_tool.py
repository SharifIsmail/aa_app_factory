...
from service.law_core.models import TaskStatus, ToolLog, WorkLog
from service.law_core.utils.text_utils import (
    clean_text,
    remove_images_from_text,
    remove_xml_metadata_from_html_content,
)
from service.metrics import web_scraping_duration_seconds
...
class VisitWebpageUserAgentTool(Tool):
    ...
    def _fetch_with_requests(self, url: str) -> str:
        ...
        try:
            ...
            content = remove_xml_metadata_from_html_content(response.text)
    ...
    def _fetch_with_curl(self, url: str) -> str:
        ...
        html_content = remove_images_from_text(html_content)
    ...
    def _process_content(
        self, content: str, url: str, tool_log: Optional[ToolLog]
    ) -> str:
        ...
        try:
            ...
            data = {
                "url": url,
                "content": content,
                "timestamp": tool_log.timestamp if tool_log else None,
            }
    ...
    def forward(self, url: str) -> str:
        ...
        # Create a tool log entry only if work_log is available
        tool_log = None
        if self.work_log:
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    result = self._process_content(content, url, tool_log)
                    success = True
                    return result
                except Exception as e:
            ...
            web_scraping_duration_seconds.labels(success=str(success)).observe(duration)


if __name__ == "__main__":
    # Create mock dependencies for testing
    data_storage = None  # Can be None for testing
    execution_id = "test_execution_123"
    work_log = WorkLog(id="test_work_log", status=TaskStatus.IN_PROGRESS, tasks=[])
    repo_key = "test_repo"

    # Initialize the tool
    tool = VisitWebpageUserAgentTool(
    ...
    # Test URL
    test_url = "https://publications.europa.eu/resource/cellar/3f6934ca-ea74-11ef-b5e9-01aa75ed71a1.0006.03/DOC_1"

    logger.info(f"Testing VisitWebpageUserAgentTool with URL: {test_url}")
    logger.info("=" * 80)
    ...
    for log in work_log.tool_logs:
        logger.info(f"Tool: {log.tool_name}")
        logger.info(f"Params: {log.params}")
        logger.info(f"Result length: {len(log.result) if log.result else 0} characters")
