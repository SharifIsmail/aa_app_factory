...
from service.storage_backend import StorageBackend
    ...
    def _get_file_id(self, prefixed_filename: str) -> Optional[str]:
        ...
        while True:
            try:
                ...
                logger.error(f"Failed to fetch filename {prefixed_filename}: {e}")
                return None
            ...
            if len(files) > 1:
                raise RuntimeError(
                    f"More than one file exists with name {prefixed_filename}: {files}"
                )

            matching_file = next(
                (file for file in files if file["name"] == prefixed_filename),
                None,
            )

            if matching_file:
                return str(matching_file["fileId"])
            ...
            for file in files:
                ...
                if filename.startswith(folder_prefix):
                    ...
                    if filename_without_prefix in filenames:
                        raise RuntimeError(
                            f"Filename '{filename_without_prefix}' exists multiple times in folder '{folder}': {files}"
                        )
